...
{	
	# ./ms_tester $@
	# DIFF=$(diff ./log/bash.log ./log/ms.log)
	# echo $DIFF
	# if [ ${#DIFF} > 0 ]; then
	...
	ARG=$@
	TEST1=$(./minishell -c $ARG 2&> /dev/null)
	ES_1=$?
	TEST2=$(/bin/bash -c $ARG 2&> /dev/null)
	echo $TEST2
	ES_2=$?
...
exec_test 'export | grep "OLDPWD"'
exec_test 'export | grep "PWD"'
exec_test 'export $? '
